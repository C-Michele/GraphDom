        ...
        for(std::size_t k = 0; k < inserted_vertices_and_incidence_matrix.size(); ++k ) {                                                                                                   \
            ...
            for(auto k_adj_list_edge_itr = k_adj_list.begin(); k_adj_list_edge_itr != k_adj_list.end(); ){                                                                                  \
                ...
                for(std::size_t j = 0; j < inserted_vertices_and_incidence_matrix.size(); ++j ){                                                                                            \
                    ...
                    std::set<std::size_t*> vertices_raw_pointers_encountered_in_j_adj_list;                                                                                                 \
                    ...
                    std::set<std::size_t*> vertices_raw_pointers_encountered_in_j_adj_list_directed;                                                                                        \
                    for(auto edge_itr = j_adj_list.begin(); edge_itr != j_adj_list.end(); ++edge_itr ) {                                                                                    \
                        ASSERT_EQ( edge_itr.edge_type() , MAIN_LIBRARY_NAMESPACE::edge_type::directed );                                                                                    \
                                                                                                                                                                                            \
                        std::size_t* const edge_itr_endpoint_raw_pointer = &( *( *( edge_itr ) ) );                                                                                         \
...
#define MULTISET_VERTEX_DIGRAPH_CORRECT_EDGES_ERASION_NOT_CONSTANT_GENERIC_VERTEX_PTR_NOT_CONSTANT_ADJ_LIST_NOT_C_METHODS(TEST_SUITE_NAME,CONCRETE_CLASS_NAME)                              \
    TEST(TEST_SUITE_NAME,correct_edges_erasion__not_constant_generic_vertex_ptr__not_constant_adj_list__not_c_methods) {                                                                    \
        ...
        for(std::size_t i = 0; i < number_of_different_vertex_values; ++i) {                                                                                                                \
            for(std::size_t j = 0; j < number_of_repetitions; ++j ) {                                                                                                                       \
                inserted_vertices_and_incidence_matrix.emplace_back( graph.insert_vertex( i ) , std::set< const std::size_t* >() );                                                         \
            }                                                                                                                                                                               \
        }                                                                                                                                                                                   \
        for(std::size_t first = 0; first < inserted_vertices_and_incidence_matrix.size(); ++first) {                                                                                        \
            for(std::size_t second = 0; second < inserted_vertices_and_incidence_matrix.size(); ++second) {                                                                                 \
