        ...
        for(std::size_t k = 0; k < inserted_vertices_and_incidence_matrix.size(); ++k ) {                                                                           \
            ...
            for(auto k_adj_list_edge_itr = k_adj_list.begin(); k_adj_list_edge_itr != k_adj_list.end(); ){                                                          \
                ...
                for(std::size_t j = 0; j < inserted_vertices_and_incidence_matrix.size(); ++j ){                                                                    \
                    ...
                    auto& expected_vertices_in_j_adj_lists = ( inserted_vertices_and_incidence_matrix[j] ).second;                                                  \
                    std::set<const std::size_t*> vertices_raw_pointers_encountered_in_j_adj_list;                                                                   \
                    for(auto j_adj_list_edge_itr = j_adj_list.begin(); j_adj_list_edge_itr != j_adj_list.end(); ++j_adj_list_edge_itr ) {                           \
                        ASSERT_EQ( j_adj_list_edge_itr.edge_type() , MAIN_LIBRARY_NAMESPACE::edge_type::directed );                                                 \
                                                                                                                                                                    \
                        const std::size_t* const j_adj_list_edge_itr_endpoint_raw_pointer = &( *( *( j_adj_list_edge_itr ) ) );                                     \
                        EXPECT_TRUE( ( vertices_raw_pointers_encountered_in_j_adj_list.emplace( j_adj_list_edge_itr_endpoint_raw_pointer ) ).second );              \
                        ASSERT_NE( expected_vertices_in_j_adj_lists.find( j_adj_list_edge_itr_endpoint_raw_pointer ) , expected_vertices_in_j_adj_lists.end() );    \
                    }                                                                                                                                               \
                    ASSERT_EQ( vertices_raw_pointers_encountered_in_j_adj_list.size() , expected_vertices_in_j_adj_lists.size() );                                  \
                }                                                                                                                                                   \
            }                                                                                                                                                       \
        }                                                                                                                                                           \
    }                                                                                                                                                               \
